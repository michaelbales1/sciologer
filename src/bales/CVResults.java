...
{
	private static final Logger logger = LogManager.getLogger(CVResults.class.getName());
...
//				System.out.println("CParams=" + CParams[CPPos]);
			...
			// Note -- the next line was removed on 2-20-11
			String explanation = "Your file has been written to " + File_Settings.outputPath + filename;
			out.println(explanation);
			// IMPORTANT: THE FOLLOWING LINE IS NEEDED IF YOU WANT THE BROWSER TO PRESENT/UPLOAD THE FILE TO THE USER
			out.println("<META http-equiv='refresh' content='0;URL=getFile?filename="+ filename +"'/>");
			// out.println(File_Settings.outputPath + filename);
			// The following line is the one Robert asked me to use in March 2011 for Google Earth in browser:
			// out.println("http://cudial1.cpmc.columbia.edu/sciologer/getFile?filename=" + filename);
			// out.println(File_Settings.outputPath + filename);
		}// null check
	} //doGet
}// class
