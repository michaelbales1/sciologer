 ...
 * @version  $Revision: 1.115 $; $Date: 2007/12/13 08:45:53 $
 * @author   Dirk Beyer
 *****************************************************************/
public class CCVisu {
  ...
  /** Brief graph layout in textual format.*/
  public final static int LAYB  = 6;
  /** Brief graph layout in textual format, with labels*/
  public final static int LAYL  = 7;
  ...
  {
	  //TODO Link back into CCVisu object if you want to run this from the command line
  }
	 public CCVisu(String[] args, Job job)
	 {
		 this.job = job;
		 JobStatus jobStatus = job.getStatus(); //Get access to the job status object which we will write to.
		 jobStatus.println("In CCVisu...");
    ...
    // Default I/O.
    //TODO Set up for servlets, don't just send out like this.
    ...
    // For layout. These are the defaults, and can be overriden by the command line parameters
    int            nrDim               = 2;
...
//    float   gravitation  = 0.001f; CURRENT
    ...
    } else if (outFormat == LAYB) {                        // Layout in text format LAYB
    	// like LAY but with fewer data elements. Added by Michael Bales
        dataWriter = new WriterDataLAYBrief (graph, out);
    } else if (outFormat == LAYL) {                        // Layout in text format LAYL
    	// like LAYB but with label information (node names) included. Added by Michael Bales
        dataWriter = new WriterDataLAYL (graph, out);
    ...
    // Note: To write to a file, specify desired output file name in the runtime parameters
    out.close();
  }
  ...
  private static int getFormat(String format) {
    ...
    if (format.equalsIgnoreCase("CVS")) {
      ...
      result = LAY;
    } else if (format.equalsIgnoreCase("LAYB")) {
        result = LAYB;
    } else if (format.equalsIgnoreCase("LAYL")) {
        result = LAYL;
   ...
   *****************************************************************/
  public void computeLayout(GraphData graph, 
                                   int nrIterations, 
                                   float attrExponent, 
                                   float repuExponent, 
                                   boolean vertRepu,
    ...
    Minimizer minimizer = new MinimizerBarnesHut(verticeNr, attrIndexes, attrValues, 
                               repu, graph.pos, fixedPos,
                               attrExponent, repuExponent, gravitation, job);
