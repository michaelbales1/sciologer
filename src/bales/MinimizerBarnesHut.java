 ...
 * @version  $Revision: 1.28 $; $Date: 2007/12/13 08:45:53 $
...
{ 
	private static final Logger logger = LogManager.getLogger(MinimizerBarnesHut.class.getName());
	...
	private Job job; //This is the query we're running and how we're going to send output.
	...
	public MinimizerBarnesHut(int nodeNr, int[][] attrIndexes, float[][] attrValues, 
			...
			float attrExp, float repuExp, float gravitationFactor, Job job) 
	{
		this.job = job;
		this.nodeNr = nodeNr;
...
//			+ "   energy " + energySum
//			+ "   repulsion " + repuFactor);
			if(step%5==0) // Print status after every 5 iterations
			info("Completed iteration " + step + " of " + nrIterations + "...");            
			//notify the listeners
