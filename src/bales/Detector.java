...
public class Detector
{
    ...
    private String umlsmapfile = "umls_with_lexical_variants.txt";
	...
	private HashMap<String, String> readHashMap()
	{
		...
		try
		{
			...
			while(reading != null)
			{
				...
				newHash.put(reading.substring(brk1+1, brk2),reading.substring(0,brk1));
...
//			System.out.println("Found: " + first + " -> " + last);
	...
	private boolean check(String toCheck)
	{
		...
		if(this.hash.containsKey(toCheck))
		{
			try
			{
				logWriter.write("Found: " + toCheck + " -> " + this.hash.get(toCheck));
	...
	{
		Detector d = new Detector("C:\\AQUA\\project\\AQUA\\bin\\umls_test_all.txt");
